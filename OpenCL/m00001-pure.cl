...
KERNEL_FQ void m00001_init (KERN_ATTR_TMPS_ESALT (pbkdf2_sha256_tmp_t, ethereum_pbkdf2_t))
{
  /**
   * base
   */
...
KERNEL_FQ void m00001_loop (KERN_ATTR_TMPS_ESALT (pbkdf2_sha256_tmp_t, ethereum_pbkdf2_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void m00001_comp (KERN_ATTR_TMPS_ESALT (pbkdf2_sha256_tmp_t, ethereum_pbkdf2_t))
{
  /**
   * base
   */
  ...
  st[12] = hl32_to_64_S (ciphertext[21], ciphertext[20]);
  st[13] = hl32_to_64_S (ciphertext[23], ciphertext[22]);
  st[14] = hl32_to_64_S (ciphertext[25], ciphertext[24]);
  st[15] = hl32_to_64_S (ciphertext[27], ciphertext[26]);
  st[16] = hl32_to_64_S (ciphertext[29], ciphertext[28]);
  st[17] = hl32_to_64_S (ciphertext[31], ciphertext[30]);
  st[18] = 0;
  st[19] = 0;
  st[20] = 0;
  st[21] = 0;
